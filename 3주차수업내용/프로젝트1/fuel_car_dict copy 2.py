...
fuel_messy_datas = list(zip(list_passenger,list_bus,list_truck,list_special))

fuel_datas = fuel_messy_datas[1:]
...
pattern_list_1 = ['승용차', '승합차', '화물차', '특수차'] # 열
pattern_list_2 = ['휘발유','경유','LPG','전기','CNG','하이브리드','수소','기타'] # 행

query = []
for row_idx, car_type in enumerate(pattern_list_1): # 열
    for col_idx, fuel_type in enumerate(pattern_list_2): # 행
        value = fuel_datas[row_idx * len(pattern_list_2) + col_idx]
        query.append((car_type, fuel_type, value))
print(query[:4])

# https://chatgpt.com/s/t_68d51a5a7c3881918d3527e3c1b51c3a