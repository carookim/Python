...
#         datas = shopv2.read_customer_data()      # $  
...
#             name = st.text_input("회원아이디", selected["회원아이디"], key="edit_id")
#             customer_id = st.text_input("회원이름", selected["회원이름"], key="edit_name")
#         else:
#             name = st.text_input("회원아이디", key="new_id")
#             customer_id = st.text_input("회원이름", key="new_name")
...
#                     if st.session_state.selected_member_index is not None:
#                         # 기존 회원 수정
#                         st.session_state.members.at[st.session_state.selected_member_index, "회원아이디"] = name
#                         st.session_state.members.at[st.session_state.selected_member_index, "회원이름"] = customer_id
#                         shopv2.update_customer_data(name,customer_id) # (name, customer_id) # $
#                         st.rerun()
#                     else:
#                         # 신규 회원 추가 (concat 사용)
#                         new_row = pd.DataFrame([{"회원아이디": name, "회원이름": customer_id}])
#                         st.session_state.members = pd.concat([st.session_state.members, new_row], ignore_index=True)
#                         shopv2.create_customer_data(name) # (customer_id) # $
#                         st.session_state.selected_member_index = None
#                         st.rerun()
#         with col_b:
#             if st.button("입력 초기화"):
